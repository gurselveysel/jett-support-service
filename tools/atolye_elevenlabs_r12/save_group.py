...
s['r12_arac_hatalari']=[dict(operation='Local source background PNG write',error='OSError: image file is truncated',resolved=True,resolution='Görsel yeniden oluşturuldu; atomik yazma ve tekrar okuma eklendi. MP3 yeniden üretilmedi.')]
...
with zipfile.ZipFile(zp,'w',zipfile.ZIP_DEFLATED,compresslevel=3) as z:
 for f in folder.rglob('*'):
  if f.is_file() and '.tmp' not in f.name and '.pending' not in f.name:z.write(f,str(Path(folder.name)/f.relative_to(folder)))
